...
const OUTPUT_FILE_PATH = path.join(__dirname, '../public/files/last-updated.json');
...
// Function to get the last commit ID from Git
function getGitCommitId(filePath) {
	try {
		const stdout = execSync(`git log -1 --format="%H" -- "${filePath}"`);
		return stdout.toString().trim();
	} catch (error) {
		console.error(`Error fetching commit ID for ${filePath}:`, error);
		return null;
	}
}
...
function processFiles() {
	...
	const lastUpdatedDates = files
		.map((relativeFilePath) => {
			...
			const commitId = getGitCommitId(fullPath);
