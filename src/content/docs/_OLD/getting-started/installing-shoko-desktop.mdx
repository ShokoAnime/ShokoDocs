...
1. Install Bottles (https://usebottles.com). This requires installing Flatpak and adding the Flathub repository if you don't have it already.
2. Before adding a bottle, go to Bottles preferences and under the "Runners" tab, install the latest Caffe or Proton GE (confirmed working). Other runners may be slow or not working at all.
3. Add a Bottle and give it a name like "Shoko Desktop". Under environment, select "Custom" and choose one of the runners that were installed in the last step. Then hit "Create".
