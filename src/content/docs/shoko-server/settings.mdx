...
The Settings section allows you to customize the data collection and metadata handling for your series in Shoko. It's
organized into six main categories: General, Import, AniDB, Metadata Sites, User Management, and API Keys. These
settings enable you to control the type and quantity of information Shoko collects, ensuring that it downloads and
stores only what's necessary. This approach helps prevent the accumulation of unwanted metadata, streamlining
your collection's overall metadata size. Each section offers a range of options, from simple operational changes to
in-depth data and user management. This lets you customize Shoko's performance to suit your needs.

## General

![Shoko Server - Settings - General](../../../assets/img/shoko-server/shoko-server-settings-general.jpg)
...
For those interested in creating a unique interface, the Custom Theme section provides detailed instructions. Theme
descriptions provide author credits for support contact and issue reporting, along with the ability to manually check
for possible theme update.
...
![Shoko Server - Settings - Import](../../../assets/img/shoko-server/shoko-server-settings-import.jpg)
...
<EasyTable
  ...
  data={[
    [
      'Import on Start',
      'When enabled, Shoko will start importing files when the program is first launched.',
    ],
    [
      'Rename on Import',
      'When enabled and there is a renamer configured, Shoko will rename imported files.',
    ],
    [
      'Move on Import',
      'When enabled and when there is both a source and destination import folder. Shoko will move files to the ' +
      'destination import folder',
    ],
    [
      'Move After Rename',
      'When enabled and when there is a renamer configured and there is both a source and destination import folder, ' +
      'Shoko will rename the file first and then move it.',
    ],
    [
      'Set File as Watched if Episode is Watched',
      'When MyList functionality is enabled, an episode marked as watched on AniDB will have the file marked as ' +
      'watched when imported.',
    ],
    [
      'Video Extensions',
      'All the formats Shoko will import within the set import folders.',
    ],
  ]}
  center={true}
...
![Shoko Server - Settings - AniDB](../../../assets/img/shoko-server/shoko-server-settings-anidb.jpg)
...
![Shoko Server - Settings - Metadata Sites](../../../assets/img/shoko-server/shoko-server-settings-metadata.jpg)
...
These settings determine if additional images, such as posters and fanart will be downloaded for the series in your
collection. If enabled, you can also determine how many images of each type you want to download for each series.

### TVDB Options

TVDB serves as one of our primary metadata providers, offering both data and images for the series in your collection.
While it's not mandatory, we strongly recommend enabling the Auto Link setting to ensure that your collection benefits
from episode data. You can also specify the language in which you want TVDB data to be downloaded. However, if the
data is not available in your selected language, it will be downloaded in English. Additionally, extra images like
posters, fan art, and banners will be fetched for the series in your collection. If you choose to enable this feature,
...
![Shoko Server - Settings - User Management](../../../assets/img/shoko-server/shoko-server-settings-users.jpg)
...
:::caution[Restricting Tags]
Any tag added here takes precedence over any other settings related to series viewing no matter what is set.
:::

## API Keys

![Shoko Server - Settings - API Keys](../../../assets/img/shoko-server/shoko-server-settings-api.jpg)
