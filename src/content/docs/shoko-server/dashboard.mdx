...
The Queue Processor is the comprehensive task runner responsible for processing all tasks related to your collection.
It is divided into three workers: **Hasher**, **General**, and **Images**, with each worker handling specific tasks.

### Hasher

The hasher worker manages all file-related hashing tasks, which are necessary before any of the subsequent tasks can
be executed. Our hashing system utilizes the [rHash](https://github.com/rhash/RHash) library for hashing, as it is cross-platform and generally performs
well, depending on your CPU and available I/O.

### General

The general worker is responsible for handling every task that is not related to hashing or image downloading, which
constitutes the majority of all tasks that Shoko will perform while building and managing your collection. It is not
unusual for the number of tasks in this category to increase while commands are being executed or even appear to
remain unchanged. This simply indicates that more related tasks were added than processed at that time.

### Images

The images worker handles all image downloading for the series in your collection. The amount of images downloaded per
series will depend on your **AniDB** and **Metadata** settings. It is not unusual for this number to be in the hundreds
or even thousands during import processing.
