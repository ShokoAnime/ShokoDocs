...
With Shokofin, setting the preferred download language or country does nothing unless you set any overrides within the
plugin settings. (? This needs to be verified)
...
When using the VFS feature provided by Shokofin, it is recommended to leave this option disabled as the artwork would end up saved into the symlinked file structure that Shokofin creates within the data directories of Jellyfin. For non-VFS managed libraries, this is not an issue and can be set according to personal preference.
