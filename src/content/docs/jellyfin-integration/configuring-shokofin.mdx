...
<EasyTable
  ...
  data={[
    ...
    {
      Option: "Add trailers",
      Description: "Makes trailers appear as a native feature that Jellyfin can use. This setting is only applicable when using a [VFS](#virtual-file-system-vfs) managed library.",
      Default: "&#x2713;"
    },
    {
      Option: "Add credits as theme videos",
      Description: "Make OPs/EDs appear as a native theme video that Jellyfin can use. This setting is only applicable when using a [VFS](#virtual-file-system-vfs) managed library.",
      Default: "&#x2713;"
    },
    {
      Option: "Add credits as special features",
      Description: "Make OPs/EDs show up as native special features within Jellyfin. This setting is only applicable when using a [VFS](#virtual-file-system-vfs) managed library.",
      Default: ""
    },
    {
...
In this section, you will see some settings that are actively being developed. With that does come the risk of strange
or inefficient behavior when changing any of these settings. Be sure to read what each option does before changing so
you have an idea what's relevant to you as well as what to expect.

##### Automatically Merge Versions
![Shokofin - Plugin Settings - Experimental (1)](/src/assets/img/shokofin/Shokofin-Settings-Experimental-1.png)



##### Split Existing Versions Before Merging
![Shokofin - Plugin Settings - Experimental (2)](/src/assets/img/shokofin/Shokofin-Settings-Experimental-2.png)


##### Automatically Merge Seasons
![Shokofin - Plugin Settings - Experimental (3)](/src/assets/img/shokofin/Shokofin-Settings-Experimental-3.png)

The problem this setting is trying to solve is when a season of an anime is aired, but is broken up into multiple parts.
This can be due to the parts not airing in consecutive time slots among many other reasons, but the end result is that
AniDB keeps track of each part as their own entry. Some well known examples of this are Attack on Titan, Mushoku Tensei,
and Spy x Family.

Enabling this option will allow Shokofin to try and detect which seasons are broken up into parts, and merge them into a
single season entry. At the moment this feature is fairly experimental, and has no easy way to configure for the average
user. Any change to this setting will require you to refresh the library and replace all metadata for the new changes to
take effect when using VFS managed libraries. This feature is untested for non-VFS managed libraries, so it may not work
and may even warrant a library recreation for the slight possibility that it does.
