...
:::tip
Make sure Plex Media Server is stopped before attempting to install the Agents and Scanners. Otherwise, a restart may be necessary to make the agents and scanners work and appear in your Library settings.
:::

:::caution[Nvidia Shield Compatibility]
As detailed [here](https://forums.plex.tv/t/important-information-for-users-running-plex-media-server-on-nvidia-shield-devices/883484), these plugins will not function when running Plex Media Server v1.41.0 or newer on an Nvidia Shield device. It is possible to sideload an older PMS build to continue to use third-party plugins, but that will not be covered here.
:::
...
Below are the three most common installation methods, and what you need to do for each one before you continue with the rest
of the guide.
