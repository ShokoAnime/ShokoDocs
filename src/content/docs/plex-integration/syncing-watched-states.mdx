...
3. Configure Plex Users
    - Configure the usernames of the Plex accounts you would like to sync with Shoko.
...
:::caution[Cross Platform Compatibility]
...
Syncing via utility script is the most robust method as it works cross platform, and with any episode type. As a caveat, it also requires the most effort and knowledge to set up.
...
Using Task Sceduler on Windows, Cron on Linux, or any other automation equivalent to run the `watched-sync.py` script on a schedule allows this method to be "Set and Forget".
:::
