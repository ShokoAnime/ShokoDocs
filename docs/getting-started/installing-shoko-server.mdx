...
```
After pulling the image, you can run the command below. Mare sure you substitute the paths under <code>-v</code> with the ones from your environment.
...
As the management using Container Station will feel a bit clunky, we recommend starting the container using the instructions given under **Docker commandline** above.
...
Docker is not supported on this platform, as **TrueNAS Core** uses FreeBSD as base OS.
